...
inline int cmp5(const void *p1, const void *p2){
        char* a1=*(char**)p1;
        char* a2=*(char**)p2;
	int len1=strlen(a1);
	int len2=strlen(a2);
	int len=len1>len2?len1:len;
	int off=(len+15)>>4;
#if 1
	switch(off){
	case 1:
...
#define write_bytes_table(len) write_bytes_###len
...
int main(int argc ,char** argv){
	...
	for(int i=0;i<size;++i){
		...
		mycpy(addr1+index,lines[i],len+1);
	...
	gettimeofday(&tv10,0);
	fprintf(stderr,"output interval=%d\n",(tv10.tv_sec-tv9.tv_sec)*1000+(tv10.tv_usec-tv9.tv_usec)/1000);
	//munmap(addr1,length);
//	close(ifd);
	close(ofd);
//	munmap(addr,length);
